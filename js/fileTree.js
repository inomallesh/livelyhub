...
import { openFile, closeTab, getActiveFileId, onActiveFileChange } from "./fileEditor.js";
  ...
  if (actionBtn) {
    ...
    if (action === "rename") {
      ...
      if (confirm(`Delete ${label}? This can't be undone.`)) {
        // Close any open tab(s) for this file, or every descendant file if
        // this is a folder — closeTab() is a safe no-op for files that
        // aren't currently open.
        const toClose = new Set([fileId]);
        let changed = true;
        while (changed) {
          changed = false;
          for (const f of files) {
            if (toClose.has(f.parentId) && !toClose.has(f.fileId)) {
              toClose.add(f.fileId);
              changed = true;
            }
          }
        }
